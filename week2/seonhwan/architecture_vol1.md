...
## 단일 서버
![](./image/./image/Pasted%20image%2020251111195549.png)
기본적인 흐름
- 도메인 이름을 이용해 웹사이트 접속 (DNS 이용)
- IP 주소로 HTTP 요청
- HTML, JSON 형태의 응답을 반환
## 데이터베이스
![](./image/Pasted%20image%2020251111195812.png)
### 데이터베이스 종류
- 관계형 데이터 베이스(RDBMS)
...
### 로드 밸런서
![](./image/Pasted%20image%2020251111200418.png)
...
#### 데이터베이스 다중화
![](./image/Pasted%20image%2020251111200918.png)
...
![](./image/Pasted%20image%2020251111201840.png)
읽기 주도형 캐시 전략
...
![](./image/Pasted%20image%2020251111202610.png)
### 고려사항
- 비용
...
![](./image/Pasted%20image%2020251111203833.png)
...
![](./image/Pasted%20image%2020251111204559.png)
## 데이터베이스 규모 확장
- 앞서 설명했던 것처럼 스케일 아웃이 더 적합하다.
- 샤딩
	- 대규모 데이터베이스를 샤드라는 단위로 분할하는 기술
	- 샤드에 보관되는 데이터는 중복되지 않는다
	![](./image/Pasted%20image%2020251111204738.png)
	- 가장 중요한 것은 샤딩키
...
- 처리율 제한 장치의 위치
	...
	- 서버에 두기
	![](./image/Pasted%20image%2020251111210015.png)
	- 미들웨어로 두기
	![](./image/Pasted%20image%2020251111210048.png)
...
- 요청 처리 시 하나의 토큰 사용한다.
	...
	- 충분하지 않은 경우 : 버려진다.
	![](./image/Pasted%20image%2020251111210604.png)
![](./image/Pasted%20image%2020251111210638.png)
- 이는 2개의 인자를 받는다
...
- 동작 원리
	...
	![](./image/Pasted%20image%2020251111211150.png)
- 인자
	- 버킷 크기 : 큐 사이즈와 같은 값
...
- 작동 방식
	...
	![](./image/Pasted%20image%2020251111213809.png)
- 장점
...
#### 이동 윈도 카운터 알고리즘
- 고정윈도 카운터 + 이동 윈도 로깅
![](./image/Pasted%20image%2020251111214207.png)
...
- EXPIRE : TTL을 설정한다
![](./image/Pasted%20image%2020251111214753.png)
...
![](./image/Pasted%20image%2020251111215054.png)
...
### 상세 설계
![](./image/Pasted%20image%2020251111215327.png)
...
- 경쟁 조건
	...
	- 넘지 않으면 값 1 증가
![](./image/Pasted%20image%2020251111215742.png)
...
- 동기화 이슈
	...
	![](./image/Pasted%20image%2020251111220808.png)
	- 고정세션
		- 요청이 항상 같은 처리율 장치로 보낼 수 있도록 하는 것
		- 확장가능하지 않고 유연하지 않아 추천하지 않는다
	- 레디스를 사용해 해결한다
	![](./image/Pasted%20image%2020251111220903.png)
## 4단계 : 마무리
- 경성 처리율 제한
