...
- 단축 URL을 생성할 때 한 번 이상 데이터베이스 질의를 해야 함 -> 오버헤드가 크다. 
- ```블룸 필터``` 사용 시 성능 ⬆️
...
- 62진법 : hashValue에 사용할 수 있는 문자 개수가 62개이기 때문. 
- (ex.)
...
> 3. DB에 URL이 없으면 해당 URL은 새로 접수된 것 -> **유일한 ID 생성**. 이 ID는 **데이터베이스의 기본 키**로 사용
> 4. 62진법 변환 적용, ID를 단축 URL로 만든다
> 5. ID, 단축 URL, 원래 URL로 새 데이터베이스 레코드를 만든 후 단축 **URL을** 클라이언트에 전달 
...
### URL 리디렉션 상세 설계
- 쓰기<<읽기 -> <단축URL, 원래 URL>의 쌍을 캐시에 저장하여 성능을 높임
...
> 4. 캐시에 해당 단축 URL이 없는 경우에는 데이터베이스에서 꺼냄. 데이터베이스에 없다면 사용자가 잘못된 단축 URL을 입력한 경우.
